        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.status === 'success') {
                 ...
                 console.log("Contenu du panier:", localStorage.getItem("panier"));
                } else {
                ...
                panier.forEach((produit) => {
                    ...
                    panierContainer.innerHTML += `
                        <div class="panier-item d-flex justify-content-between align-items-center border p-2 mb-2">
                            <img src="../assets/images/${produit.image}" alt="${produit.name}" width="50">
                            <span>${produit.name}</span>
                            <span>${produit.price} €</span>
                            <div>
                                <button class="btn btn-sm btn-danger retirer" data-id="${produit.id_produit}">➖</button>
                                <span>${produit.quantité}</span>
                                <button class="btn btn-sm btn-success ajouter" data-id="${produit.id_produit}">➕</button>
                            </div>
                        </div>
                    `;
                });
    ...
    document.addEventListener("click", function (event) {
        if (event.target.classList.contains("ajouter") || event.target.classList.contains("retirer")) {
            ...
            $.ajax({
                ...
                type: 'POST',
                data: { id_produit: id, action: action },
                dataType: 'json',
                success: function(response) {
                    if (response.status === 'success') {
                        afficherPanier();
